...
if($_GET['idh'])
{
	...
	$query_tx = MySQLi_Query($spojeni,"SELECT *  FROM homilie WHERE $aktivni=1 AND id='".intval($_GET['idh'])."' ") or die(err(1));
	$row_tx = MySQLi_fetch_object($query_tx);
	   ...
	   if($row_tx->pozadi)
		...
		// vypis podkategorii
			...
			$query_t = MySQLi_Query($spojeni,"SELECT id, $nazev, $str, vnor, id_nadrazeneho, typ, typ2, pozadi, ico, $autor FROM homilie WHERE $aktivni=1 AND id_nadrazeneho='".intval($_GET['idh'])."'  ORDER BY nazev_cz, id") or die(err(1));
		    while($row_t = MySQLi_fetch_object($query_t))
		    {
				...
				if($row_t->vnor==4)
				{
					...
					if($row_t->typ2==1 && $h==0)
					{
						...
						texty_homilie($_GET['idh'],$spojeni);
						echo "</article>";
				...
				elseif($row_t->vnor==2)
				{
				   ...
				   echo "<div class='col-6 col-xs-6 col-sm-6 col-md-4 col-lg-3'>
						<a class='homilie-single' href='/".__LANG__."/homilie/".$row_tx->$str."/".$row_t->$str."/".$row_t->id.".html'>
						<img alt='".stripslashes($row_t->$nazev)."' src='/img/".$row_t->ico."'  title='".stripslashes($row_t->$nazev)."'>
						<p>
						".stripslashes($row_t->$nazev)."
						</p>
...
<div class='row'>";
	...
	$query_t = MySQLi_Query($spojeni,"SELECT id, $nazev, $str, ico FROM homilie WHERE $aktivni=1 AND vnor=1 AND typ2=0  ORDER BY $nazev, id") or die(err(1));
    while($row_t = MySQLi_fetch_object($query_t))
    {
		
		echo "<div class='col-12 col-xs-6 col-sm-6 col-md-4 col-lg-4'>
		      <div class='homilie-single'>
				<a  href='/".__LANG__."/homilie/".$row_t->$str."/".$row_t->id.".html'>
				<img alt='".stripslashes($row_t->$nazev)."' src='/img/".$row_t->ico."'  title='".stripslashes($row_t->$nazev)."'>
				<p>
				".stripslashes($row_t->$nazev)."
				</p>
